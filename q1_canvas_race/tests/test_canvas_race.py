...
2. Q1-A/B: test_pixel_state_transition_detected_under_fibonacci_jitter (ASYNC)
3. Q1-H/I: test_pixel_engine_dynamic_calibration_adapts_to_canvas_resize (ASYNC)
4. Q1-F/G: test_corrupted_payload_silently_accepted_when_boundary_disabled (ASYNC)
5. Q1-N: test_circuit_breaker_recovers_from_coordinate_drift
6. Q1-N: test_circuit_breaker_exhaustion_opens_circuit
7. Q1-K/L/M: test_chained_actions_land_inside_race_window (20-run latency report, ASYNC)
"""
...
async def browser_instance():
    async with async_playwright() as p:
        ...
        yield browser
        # Browser is closed automatically when exiting context manager
...
@pytest.mark.asyncio
async def test_pixel_state_transition_detected_under_fibonacci_jitter(browser_instance):
    """Q1-A/B: Verifies real browser WebSocket route interception & pixel state engine under Fibonacci jitter."""
    context = await browser_instance.new_context()
    
    # REQUIREMENT: max_jitter_ms=8000.0 must be used for official test
    interceptor = WSChaosInterceptor(ChaosConfig(enable_jitter=True, max_jitter_ms=8000.0))
    # REQUIREMENT: Route registered on context BEFORE page navigation to avoid race conditions
    await interceptor.attach_to_context_async(context)

    page = await context.new_page()
    await page.goto("http://localhost:8081/?seed=101&boundary=on")

    pixel_engine = PixelEngine(page)
    # Calibrate will wait until the grid renders
    await pixel_engine.calibrate(25000.0)

    sample = await pixel_engine.wait_for_cell_active(0, 25000.0)
...
@pytest.mark.asyncio
async def test_pixel_engine_dynamic_calibration_adapts_to_canvas_resize(browser_instance):
    """Q1-H/I: Resizes canvas via JS, re-runs calibrate(), and asserts derived cell centers CHANGED."""
    context = await browser_instance.new_context()
    page = await context.new_page()
    await page.goto("http://localhost:8081/?seed=102&boundary=on")

    pe = PixelEngine(page)
    initial_offsets = await pe.calibrate()
    c0_initial = initial_offsets[0]
    details_initial = await pe.get_calibration_details()
    ...
    resized_offsets = await pe.calibrate()
    c0_resized = resized_offsets[0]
    details_resized = await pe.get_calibration_details()
...
@pytest.mark.asyncio
async def test_corrupted_payload_silently_accepted_when_boundary_disabled(browser_instance):
    """Q1-F/G: Loads boundary=off, injects 1e+7 through WS route, asserts NO error glyph renders and framework detects corruption."""
    context = await browser_instance.new_context()
    interceptor = WSChaosInterceptor(ChaosConfig(enable_mutation=True, target_mutation_seq=1, mutation_type="OVERFLOW", max_jitter_ms=0.0))
    await interceptor.attach_to_context_async(context)

    page = await context.new_page()
    await page.goto("http://localhost:8081/?seed=505&boundary=off")

    # Give time for mutation payload to be processed
    await asyncio.sleep(1.0)

    # Check canvas error glyph region (Y=25) -> when boundary=off, NO red error banner is rendered
    glyph_rendered = await page.evaluate("""
    () => {
        const canvas = document.querySelector('#terminal');
    ...
    cell_value_corrupted = await page.evaluate("""
    () => {
...
@pytest.mark.asyncio
async def test_chained_actions_land_inside_race_window(browser_instance):
    """Q1-K/L/M: Measures latency to START of hover action, asserting latency <= 100ms across 20 iterations."""
    latencies = []
    passed_count = 0
    failed_count = 0

    # REQUIREMENT: 20 real race runs
    for i in range(20):
        context = await browser_instance.new_context()
        # Using 8000ms max jitter for official test as required
        interceptor = WSChaosInterceptor(ChaosConfig(enable_jitter=True, max_jitter_ms=8000.0))
        await interceptor.attach_to_context_async(context)

        page = await context.new_page()
        await page.goto(f"http://localhost:8081/?seed={200 + i}&boundary=on")

        pe = PixelEngine(page)
        grid = await pe.calibrate(25000.0)

        # Wait up to 25s to allow for the 8s maximum jitter frames to pass
        sample = await pe.wait_for_cell_active(0, 25000.0)
        detection_py_ns = time.perf_counter_ns()

        dispatcher = ChainedActionDispatcher(min_window_ms=0.0, max_window_ms=100.0)
        result = await dispatcher.fire(page, grid[0], detection_py_ns)
